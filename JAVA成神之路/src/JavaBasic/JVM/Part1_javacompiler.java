...
public class Part1_javacompiler {
    public static void main(String[] args) throws Exception {
        ...
        String code = "public class Hello{" +
                ...
                "}" +
                "}";
        File file = new File("xxxjava/Hello.java");
        if (!file.exists()) {
        ...
        int result2 = compiler.run(null, null, null, "xxxjava/Hello.java");
        System.out.println(result == 0 ? "编译成功" : "编译失败");
        System.out.println(result2 == 0 ? "编译成功" : "编译失败");

//通过Runtime类动态运行编译好的类
        Runtime rt = Runtime.getRuntime();
        Process pro = rt.exec("java -cp xxxjava Hello");//实际上已经执行了
